...
export function generateStaticParams() {
  const limit = process.env.PREBUILD_ALL === 'true' ? null : Number(process.env.PREBUILD_COMPARISON_LIMIT || 20);
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${first.name} vs ${second.name} : comparatif complet`,
    description: `Comparez ${first.name} et ${second.name} : prix, fonctionnalités, cas d’usage, avantages, limites, notes et alternatives.`,
  ...
  return (
    <>
      ...
      <div className="container page-shell">
        ...
        <p className="lead">Comparaison automatique basée sur les données JSON : prix, catégorie, note, fonctionnalités, cas d’usage, avantages, limites et sources officielles.</p>
        ...
        <section className="section-block cta-box">
          ...
          <p>
            Choisissez <strong>{first.name}</strong> si votre priorité est alignée avec {first.useCases?.slice(0, 2).join(' et ')}. Choisissez <strong>{second.name}</strong> si vous recherchez plutôt {second.useCases?.slice(0, 2).join(' et ')}.
          </p>
