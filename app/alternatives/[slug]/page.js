...
export function generateStaticParams() {
  const limit = process.env.PREBUILD_ALL === 'true' ? Infinity : Number(process.env.PREBUILD_ALTERNATIVES_LIMIT || 10);
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Alternatives à ${tool.name} : comparatif des meilleurs concurrents`,
    description: `Découvrez les meilleures alternatives à ${tool.name}, avec prix, fonctionnalités, cas d’usage, avantages, limites et comparatifs.`,
  ...
  return (
    <>
      ...
      <div className="container page-shell">
        ...
        <p className="lead">Comparez {tool.name} avec des solutions proches selon la catégorie, le type, la note, les cas d’usage et les fonctionnalités.</p>
        ...
        <section className="section-block">
          ...
          <div className="link-grid">
            {alternatives.slice(0, 12).map((alternative) => (
              <Link key={alternative.slug} href={`/comparatif/${tool.slug}-vs-${alternative.slug}`} className="comparison-link">
